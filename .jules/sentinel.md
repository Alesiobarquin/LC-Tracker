...
## 2024-05-24 - Fix XSS bypass in nested image src attributes
**Vulnerability:** The Admin Dashboard sanitized user-provided URLs for anchor `href` attributes but passed the raw, unsanitized URL to the nested `<img>` `src` attribute.
**Learning:** Sanitizing the container element is insufficient if nested elements that load resources (like images) still use raw input, as it allows attackers to bypass XSS protections via the nested tag.
**Prevention:** Ensure that any nested resource-loading tags (such as `<img>` `src` attributes) also strictly use the sanitized URL rather than the raw input.
