...
export const Dashboard: React.FC = () => {
  ...
  // We moved the early return down to obey hooks rule


  const newProblemData = useMemo(() => effectiveNewProblemId ? problemMap[effectiveNewProblemId] || null : null, [effectiveNewProblemId]);
  const reviewProblemsData = useMemo(() => effectiveReviewProblems.map(id => problemMap[id]).filter(Boolean), [effectiveReviewProblems]);
  const coldSolveData = useMemo(() => coldSolveProblem ? problemMap[coldSolveProblem] || null : null, [coldSolveProblem]);
  const syntaxDrillsData = useMemo(() => (dueSyntaxCards || []).map(id => syntaxCardMap[id]).filter(Boolean), [dueSyntaxCards]);
