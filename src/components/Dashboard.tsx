        ...
        <div className="premium-card p-4 sm:p-6 border-amber-500/30 bg-amber-500/5 relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative z-10">
            ...
            <div className="flex flex-wrap gap-2 w-full sm:w-auto">
              ...
              <button onClick={dismissCatchUpBanner} className="px-2 py-2 text-zinc-500 hover:text-zinc-300 rounded-lg hover:bg-zinc-800/50 transition-colors" aria-label="Dismiss catch up banner" title="Dismiss banner"><X size={18} /></button>
            </div>
          </div>
        </div>
      )}
                  ...
                  <div className="mt-8 flex flex-col md:flex-row md:items-start gap-6">
                    <div className="flex-1">
                      ...
                      <div className="flex gap-2 mt-4">
                        <a href={newProblemData.leetcodeUrl} target="_blank" rel="noopener noreferrer" aria-label={`Open ${newProblemData.title} on LeetCode`} title={`Open ${newProblemData.title} on LeetCode`} className="p-2.5 bg-zinc-800/80 hover:bg-zinc-700 rounded-xl text-zinc-300 transition-colors border border-zinc-700/50"><ExternalLink size={16} /></a>
                    ...
                    <div key={prob.id} className={clsx('premium-card p-6 relative overflow-hidden group border', isPrimary ? 'border-emerald-500/20' : 'border-amber-500/20')}>
                      ...
                      <div className={clsx('flex justify-between items-start mb-4', (isPrimary && recommendationReason) || !isPrimary ? 'mt-8' : '')}>
                        ...
                        <div className="flex gap-2">
                          ...
                          <a href={prob.leetcodeUrl} target="_blank" rel="noopener noreferrer" aria-label={`Open ${prob.title} on LeetCode`} title={`Open ${prob.title} on LeetCode`} className="p-2.5 bg-zinc-800/80 hover:bg-zinc-700 rounded-xl text-zinc-300 transition-colors border border-zinc-700/50 hover:border-zinc-600"><ExternalLink size={18} /></a>
                        </div>
                      </div>
                      <button
                  ...
                  return (
                    <div key={prob.id} className="premium-card p-4 flex items-center justify-between group">
                      ...
                      <button
                        ...
                        title={`Start ${prob.title}`}
                        aria-label={`Start ${prob.title}`}
                      >
