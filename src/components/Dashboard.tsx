...
export const Dashboard: React.FC = () => {
  ...
  // Bolt Optimization: Replace O(N) array .find() with O(1) dictionary lookups
  const newProblemData = effectiveNewProblemId ? problemMap[effectiveNewProblemId] || null : null;
  const reviewProblemsData = effectiveReviewProblems.map(id => problemMap[id]).filter(Boolean);
  const coldSolveData = coldSolveProblem ? problemMap[coldSolveProblem] || null : null;
