...
export const Dashboard: React.FC = () => {
  ...
  const newProblemData = effectiveNewProblemId ? problemMap.get(effectiveNewProblemId) : null;
  const reviewProblemsData = effectiveReviewProblems.map(id => problemMap.get(id)).filter(Boolean);
  const coldSolveData = coldSolveProblem ? problemMap.get(coldSolveProblem) : null;
  ...
  if (activeSession) {
    const problem = problemMap.get(activeSession.problemId);
          ...
          <section>
            ...
            {isRetro && newProblemData ? (() => {
              ...
              <div className="space-y-4">
                {[newProblemData, ...(additionalProblems || []).map(id => problemMap.get(id)).filter(Boolean)].map((prob, idx) => {
                  if (!prob) return null;
