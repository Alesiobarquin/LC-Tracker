...
export const Dashboard: React.FC = () => {
  ...
  // Bolt Optimization: Replaced O(N) .find() with O(1) hash map lookup
  const newProblemData = effectiveNewProblemId ? problemMap[effectiveNewProblemId] : null;
  // Bolt Optimization: Replaced O(N^2) .find() in .map() loop with O(N) hash map lookup
          ...
          <section>
            ...
            {isRetro && newProblemData ? (() => {
              ...
              <div className="space-y-4">
                {// Bolt Optimization: Replaced O(N^2) .find() in array spread with O(N) hash map lookup
