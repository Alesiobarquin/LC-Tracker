                            ...
                            <div className="pt-4 border-t border-zinc-800/50">
                                ...
                                <div className="flex gap-2 mb-3">
                                    ...
                                    <button
                                        ...
                                        title={(!newBlackoutStart || !newBlackoutEnd) ? "Select start and end dates to add" : "Add blackout date"}
                                        onClick={handleAddBlackoutDate}
                                        disabled={!newBlackoutStart || !newBlackoutEnd}
                                        className="p-2 bg-zinc-800 hover:bg-zinc-700 disabled:opacity-50 text-zinc-300 rounded-xl transition-colors shrink-0 outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
                                    >
                                        <Plus size={20} />
                                    </button>
                                </div>
                    ...
                    <section className="premium-card p-6 border-emerald-500/20">
                        ...
                        <div className="space-y-4">
                            <div className="flex flex-col sm:flex-row gap-2">
                                ...
                                />
                                <button
                                    aria-label="Add target event"
                                    title={(!newEventTitle || !newEventDate) ? "Fill event title and date to add" : "Add event"}
                                    type="button"
                                    onClick={handleAddEvent}
                                    disabled={!newEventTitle || !newEventDate}
                                    className="p-2 bg-zinc-800 hover:bg-zinc-700 disabled:opacity-50 text-zinc-300 rounded-xl transition-colors shrink-0 flex justify-center items-center outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
                                >
                                    <Plus size={20} />
                                </button>
                            </div>
                            ...
                            <div className="space-y-2 mt-4">
                                {targetEvents.map((event) => (
                                    <div key={event.id} className="flex justify-between items-center bg-zinc-900 border border-zinc-800/50 p-3 rounded-lg">
                                        ...
                                        <button aria-label={`Remove target event: ${event.title}`} title={`Remove ${event.title}`} onClick={() => removeTargetEvent(event.id)} className="text-red-400 hover:bg-red-500/10 p-2 rounded-lg transition-colors outline-none focus-visible:ring-2 focus-visible:ring-red-500"><X size={16} /></button>
                                    </div>
                                ))}
