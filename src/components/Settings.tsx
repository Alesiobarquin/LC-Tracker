                            ...
                            <div className="pt-4 border-t border-zinc-800/50">
                                ...
                                <div className="space-y-2">
                                    {settings.studySchedule.blackoutDates.map((date, idx) => {
                                        const dateRangeStr = `${new Date(date.start).toLocaleDateString()} to ${new Date(date.end).toLocaleDateString()}`;
                                        return (
                                        <div key={idx} className="flex justify-between items-center bg-zinc-900 border border-zinc-800/50 p-2 rounded-lg text-sm text-zinc-300">
                                            ...
                                            <button aria-label={`Remove blackout date from ${dateRangeStr}`} onClick={() => handleRemoveBlackoutDate(idx)} className="text-red-400 hover:bg-red-500/10 p-1 rounded-md transition-colors outline-none focus-visible:ring-2 focus-visible:ring-red-500"><X size={14} /></button>
                                        </div>
                                        );
                                    })}
                            ...
                            <div className="space-y-2 mt-4">
                                {targetEvents.map((event) => (
                                    <div key={event.id} className="flex justify-between items-center bg-zinc-900 border border-zinc-800/50 p-3 rounded-lg">
                                        ...
                                        <button aria-label={`Remove target event: ${event.title}`} onClick={() => removeTargetEvent(event.id)} className="text-red-400 hover:bg-red-500/10 p-2 rounded-lg transition-colors outline-none focus-visible:ring-2 focus-visible:ring-red-500"><X size={16} /></button>
                                    </div>
                                ))}
